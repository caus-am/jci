...
if [ "$#" -lt 1 ]; then
	echo 'Usage: query_ancestors.sh <p> <inputs...>'
	echo 'Example: query_ancestors.sh <p> <inputs...>'
else
	# number of variables
	p=$1
	shift
	...
	for i in `seq 0 $pp`; do
		for j in `seq 0 $pp`; do
			if [ "$i" -ne "$j" ]; then
				a=`query.sh "ancestor($i,$j,0)." $@` 2> /dev/null
				b=`query.sh ":-ancestor($i,$j,0)." $@` 2> /dev/null
				if [ "$a" == "Inf" ]; then
					if [ "$b" == "Inf" ]; then
